...
const ProfitStr   = ({n}) => { const r=roundY(n); const s={fontWeight:"inherit",fontFamily:"inherit",letterSpacing:"inherit"}; if(r>0) return <>{`+${r.toLocaleString()}`}<span style={s}>円</span></>; if(r<0) return <>{`-${Math.abs(r).toLocaleString()}`}<span style={s}>円</span></>; return <>±0<span style={s}>円</span></>; };
